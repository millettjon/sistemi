...
(defn sistemi-data
  ...
  (merge
    ...
    {:access_data {:user_id (access_info :user-id) :password (access_info :password)
                   :license_number (access_info :access-key) :account_number (access_info :account-number)
                   :lang_locale "en-US"}}
    ...
    {:shipping_option_data {:code "11" :description "UPS Standard"}}
    ...
    {:label_data {:label_print_code "GIF" :http_user_agent "Mozilla/4.5" :label_image_code "GIF"}}
    ; What insurance ranges should we apply?
    {:insurance_data {:currency_code "EUR" :value "50.00"}}
    {:fabricator_address_data {:address1 "ZA la Croisette" :city "Clelles en Trièves" :state_province ""
...
(def customer-data
  ...
  (merge
    ...
    {:sistemi_cust_data {:customer_context_id "SistemiContextID-XX1122"}}
    )
  )
...
(def package-data
  ...
  (merge
    ; Pulled from shelf/bookcase order
    {:package_data {:unit_code "CM" :length "22" :width "20" :height "18" :weight "14.1" :unit_code "KGS"}}
    ))
