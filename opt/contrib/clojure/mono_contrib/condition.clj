...
http://groups.google.com/group/clojure/browse_frm/thread/da1285c538f22bb5"}
  clojure.mono-contrib.condition
  (:require clojure.mono-contrib.condition.Condition)
  (:import clojure.mono_contrib.condition.Condition
           clojure.lang.IPersistentMap)
  (:use (clojure.contrib
         [def :only (defvar)]
         [seq :only (separate)])))

(defvar ^:dynamic *condition*
  "While a handler is running, bound to the condition being handled")

(defvar ^:dynamic *selector*
  "While a handler is running, bound to the selector returned by the
  handler-case dispatch-fn for *condition*")

(defvar ^:dynamic *condition-object*
  "While a handler is running, bound to the Condition object whose metadata
  is the condition")

(defvar ^:dynamic *full-stack-traces* false
